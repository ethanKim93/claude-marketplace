...
**파이프라인 흐름**: `ddd-classifier → ddd-to-brd-converter → brd-reviewer → brd-to-prd-converter → prd-writer → prd-reviewer → roadmap-generator → task-runner → release-notes-generator → retro-generator`

또는 DDD 없이 직접 시작: `brd-writer → brd-reviewer → brd-to-prd-converter → ...`
...
| `/ddd-to-brd-converter` | DDD 도메인 분류 문서 → BRD 자동 변환 — 전체 Master BRD + 도메인별 BRD 생성, 서브도메인 분류·이벤트·유비쿼터스 언어 추출로 BRD 50% 자동 완성 |
