...
| `/ddd-classifier` | DDD 도메인 분류 문서 생성 — 소스코드/문서 분석으로 서브도메인 Core/Supporting/Generic 분류, 컨텍스트 맵·유비쿼터스 언어·도메인 이벤트·애그리게이트 포함 표준 문서 생성 |

### product-management
BRD → PRD → 로드맵 → 실행 → 릴리즈 → 회고의 전체 제품 관리 라이프사이클을 지원하는 스킬 모음.

**파이프라인 흐름**:
- DDD 경로: `(ddd-classifier → ddd-to-brd-converter) → brd-reviewer → brd-to-prd-converter → prd-writer → prd-reviewer → (oracle-db-designer → oracle-db-reviewer) → roadmap-generator → task-runner → release-notes-generator → retro-generator`
- 직접 시작: `brd-writer → brd-reviewer → brd-to-prd-converter → ...`

**산출물 디렉터리 구조**:
```
docs/
├── _project/                    # 프로젝트 전체 산출물
│   ├── DDD-Classification.md    # ddd-classifier 출력
│   ├── BRD-master.md            # ddd-to-brd-converter 출력 (전체)
│   └── BRD-supplement.md        # 보완 설문서
├── {domain}/                    # 도메인별 산출물 (예: user-management, payment)
│   ├── PIPELINE-STATUS.md       # 도메인 파이프라인 현황
│   ├── BRD.md                   # 도메인 BRD
│   ├── BRD-REVIEW.md
│   ├── PRD.md                   # 기능 챕터 구조 (## Feature: {name})
│   ├── PRD-REVIEW.md
│   ├── ROADMAP.md               # 통합 로드맵 (크로스 기능 의존성)
│   ├── RELEASE-NOTES-{ver}.md
│   ├── RETRO-{sprint}.md
│   └── {feature}/               # 기능별 폴더 (roadmap-generator가 생성)
│       └── TASKS.md
```

| 커맨드 | 역할 |
|--------|------|
| `/pm-pipeline` | 파이프라인 현황 진단 — 도메인별 단계 감지, 다음 스킬 자동 추천, docs/{domain}/PIPELINE-STATUS.md 관리 |
| `/brd-writer` | BRD(비즈니스 요구사항 문서) 작성 — docs/{domain}/BRD.md 생성, 경영진 승인용, SMART 목표, MoSCoW 우선순위 |
| `/ddd-to-brd-converter` | DDD 도메인 분류 문서 → BRD 자동 변환 — docs/_project/BRD-master.md + 도메인별 docs/{domain}/BRD.md 생성 |
| `/brd-reviewer` | 기존 BRD 평가 — 13섹션 완성도 분석, SMART/MoSCoW/REQ-ID 품질 검사, 개선 Q&A 문서 생성 |
| `/brd-to-prd-converter` | BRD → PRD 자동 변환 — 기능 챕터(Feature: xxx) 구조로 PRD 초안 자동 완성 |
| `/prd-writer` | 대화형 PRD 작성 (표준 13섹션 / 경량 5섹션 / PR-FAQ 선택), 섹션 7을 기능 챕터로 구조화 |
| `/prd-reviewer` | PRD 다관점 평가 — PM·Engineering·Design 3개 관점 병렬 리뷰, 관점별 Q&A 문서 생성 |
| `/roadmap-generator` | PRD 기능 챕터 파싱 → 통합 ROADMAP + 기능별 TASKS 분리 생성 — Mermaid 시각화, RICE 우선순위, 스프린트 계획 |
| `/task-runner` | docs/{domain}/{feature}/TASKS.md 기반 태스크 실행 및 상태 관리 — 의존성 기반 추천, 진행률 대시보드 |
| `/release-notes-generator` | 릴리즈 노트 자동 생성 — 기능별 TASKS 순회하여 DONE 태스크를 PRD FR·BRD 목표에 역매핑 |
